...
const SIMULATED_CONVERSATIONS: Record<string, Array<{
  stage: string;
  probability: number;
  status: string;
  speaker: string;
  ...
  alex: [
    {
      ...
      status: "Discovery",
      speaker: "Alex (AI)",
      text: "Hello! Thanks for joining. I'm Alex, your technical GTM integration assistant. I see you want to optimize your GTM flow. Let's do a technical deep dive."
    },
    {
      stage: "Discovery",
      probability: 55,
      status: "Discovery",
      speaker: "Client",
      text: "Yes. Our follow-up speed is too slow and manual. We are losing leads to competitors."
    },
    {
      stage: "Needs",
      probability: 60,
      ...
      speaker: "Alex (AI)",
      text: "I see. We can automate intake website scraping, and pass opportunity signals directly to Salesforce via WebSocket triggers.",
      buyingSignal: "Targeting manual Salesforce entry automation"
    },
    {
      stage: "Solutions",
      probability: 70,
      ...
      speaker: "Client",
      text: "What about data exfiltration security? Is lead information safe?"
    },
    {
      stage: "Objections",
      probability: 78,
      status: "Addressing Security",
      speaker: "Alex (AI)",
      text: "Our Clawpatrol Firewall scans inputs for injection vectors, redacts PII dynamically, and encrypts archival data with AES-256.",
      objection: {
        objection: "Security of customer contact records in pipeline sync",
        response: "Clawpatrol firewall ensures SOC2 compliance and automatic PII redaction."
      }
    },
    {
      stage: "Closing",
      ...
      status: "Interested",
      speaker: "Client",
      text: "Great! Let's schedule a kickoff and sandbox trial next Monday."
    },
    {
      stage: "Closed",
      probability: 98,
      status: "Closed Won",
      speaker: "Alex (AI)",
      text: "Kickoff logged. I've sent the calendar invite and sandbox access credentials. Welcome to DealFlow.AI!"
    }
  ]
};
...
function LiveContent() {
  ...
  useEffect(() => {
    ...
    const unsubCall = onSnapshot(
      ...
      (err) => {
        console.error("[Firestore] calls listener error, starting simulation:", err);
        startSimulation();
      }
    );
    ...
    const unsubTranscript = onSnapshot(
      ...
      (err) => {
        console.error("[Firestore] transcripts listener error:", err);
      }
    );
  ...
  // Live simulation update effect for demo mode
  useEffect(() => {
    if (!isDemoMode) return;

    const persona = call?.agentPersona || "alex";
    const convo = SIMULATED_CONVERSATIONS[persona] || SIMULATED_CONVERSATIONS.alex;

    const updateDemoState = (stepIdx: number) => {
      const currentSegments = convo.slice(0, stepIdx + 1).map((c, i) => ({
        speaker: c.speaker,
        text: c.text,
        timestamp: Date.now() - (convo.length - i) * 5000,
      }));

      const currentSignals = convo
        .slice(0, stepIdx + 1)
        .filter((c) => c.buyingSignal)
        .map((c) => ({ signal: c.buyingSignal }));

      const currentObjections = convo
        .slice(0, stepIdx + 1)
        .filter((c) => c.objection)
        .map((c) => c.objection);

      const lastConvo = convo[stepIdx] || convo[0];

      setCall({
        agentPersona: persona,
        currentStage: lastConvo.stage,
        dealProbability: lastConvo.probability,
        dealStatus: lastConvo.status,
      });
  ...
  }, [isDemoMode, demoStep, call?.agentPersona]);
      ...
      setTimeout(() => {
        setTranscript((prev: any) => ({
          ...
          segments: [
            ...
            {
              speaker: "System",
              text: `Screen successfully navigated to: ${navUrl}`,
              timestamp: Date.now(),
            },
          ],
        }));
  ...
  const handleEndCall = async () => {
    ...
    if (isDemoMode) {
      alert("AI meeting consultation ended gracefully.");
        ...
        <div className="flex-[2] flex flex-col bg-white rounded-xl shadow-sm border overflow-hidden">
          <div className="p-4 border-b bg-slate-50 flex justify-between items-center">
            <h2 className="font-bold flex items-center gap-2">
              <IconSmsThread className="h-4 w-4 text-primary" />
              Live Transcript
            </h2>
        ...
        <div className="flex-1 flex flex-col gap-4 overflow-hidden">
          ...
          <Card className="p-4 flex flex-col h-[30%] overflow-hidden border-green-100 bg-green-50/30">
            <h3 className="text-xs font-black uppercase text-green-700 mb-3 flex items-center gap-2">
              <IconConversionBars className="h-3 w-3" />
              Buying Signals
            </h3>
          ...
          <Card className="p-4 flex flex-col h-[30%] overflow-hidden border-yellow-100 bg-yellow-50/30">
            <h3 className="text-xs font-black uppercase text-yellow-700 mb-3 flex items-center gap-2">
              <IconAlertObjection className="h-3 w-3" />
              Objections Detected
            </h3>
            <ScrollArea className="flex-1">
              <div className="space-y-2">
          ...
          <Card className="p-4 flex-1 border-slate-200 shadow-lg">
            <h3 className="text-xs font-black uppercase text-slate-700 mb-4 flex items-center gap-2">
              <IconKpiDashboard className="h-3 w-3" />
              Agent Controls
            </h3>
            <div className="space-y-3">
              <div className="flex gap-2">
                ...
                <Button size="sm" variant="outline" className="h-8" onClick={handleNavigate} disabled={isNavigating}>
                  {isNavigating ? <Loader2 className="h-3 w-3 animate-spin" /> : <IconSend className="h-3 w-3" />}
                </Button>
              </div>
              <Button variant="outline" className="w-full justify-start text-xs h-9 gap-2" onClick={() => window.open(`/meeting-agent/summary/${callId}`, '_blank')}>
                <IconDealBrief className="h-3 w-3" />
                View Summary Page
              </Button>
              <Button 
                variant="destructive" 
