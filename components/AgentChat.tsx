...
import { IconSend } from "@/components/gtm/GtmIcons";
    ...
    <div className="flex flex-col h-full max-w-4xl mx-auto w-full border-x border-white/10">
      <header className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-black/20 backdrop-blur-md">
        <div className="flex items-center gap-3">
          <Avatar className="h-10 w-10 border border-teal-500/50">
            <AvatarFallback className="bg-teal-600 text-white">PA</AvatarFallback>
          </Avatar>
          <div>
      ...
      <div className="p-6 border-t border-white/10 bg-black/20">
        ...
        >
          <Input
            ...
            className="flex-1 bg-white/5 border-white/10 focus:border-teal-500/50 h-12 pr-12 rounded-xl"
            disabled={isLoading}
          />
          <Button
            type="submit"
            disabled={isLoading || !input.trim()}
            className="absolute right-1.5 h-9 w-9 rounded-lg bg-teal-600 p-0 hover:bg-teal-500"
          >
            {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <IconSend className="h-4 w-4" />}
          </Button>
        </form>
