...
export class DealflowVAE {
  ...
  encode(features: number[]): { mean: number[]; logVar: number[]; latent: DealflowLatentVector } {
    ...
    return {
      ...
      latent: {
        ...
        vector,
        timestamp: Date.now(),
        metadata: { source: 'dealflow-vae-encoder' },
      },
    };
  }

  // Decode latent vector back to feature space
  decode(latent: DealflowLatentVector): number[] {
  ...
  learn(dataPoint: DealflowMarketDataPoint, learningRate = 0.001): DealflowLearningState['loss']['vae'] {
  ...
  generateTextReconstruction(latent: DealflowLatentVector): string {
