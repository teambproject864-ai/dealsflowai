// GM LLM: Cross-Model Evaluation & Comparison Module
import type { GMOutput } from './gm-llm.types';
...
export class GMEvaluator {
  ...
  // Compare GM LLM output to baseline models
  compareModels(
    gmllmOutput: GMOutput,
    ...
    // Evaluate GM LLM
    const gmMetrics = this.evaluateOutput(gmllmOutput.fusedOutput, referenceContent);
    const gmPasses = this.checkThresholds(gmMetrics);
    results.push({
      modelName: 'gm-llm',
      metrics: gmMetrics,
      timestamp: Date.now(),
      contentSample: gmllmOutput.fusedOutput,
      passesThresholds: gmPasses,
    });
