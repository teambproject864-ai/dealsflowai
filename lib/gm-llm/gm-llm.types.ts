// GM LLM: Go-to-Market Large Language Model - Types

// GM LLM Core Types
export interface GMLatentVector {
  id: string;
  vector: number[];
  timestamp: number;
...
export interface GMLearningState {
  epoch: number;
  loss: {
    llm: number;
    ganGenerator: number;
...
export interface GMOutput {
  llmOutput: string;
  ganOutput: string;
  vaeOutput: { latent: GMLatentVector; reconstruction: string };
  fusedOutput: string;
  confidence: number;
  metadata: {
    timestamp: number;
...
export interface GMMarketDataPoint {
  id: string;
