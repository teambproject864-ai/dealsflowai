// GM LLM: Synthetic Data Ingestion Pipeline
import { hfInfer } from '../huggingface';
import { nvInfer } from '../nvidia';
import { kimiInfer } from '../kimi';
import type { GMMarketDataPoint } from './gm-llm.types';
...
export class GMDataIngestionPipeline {
      ...
      await Promise.all(
        enabledModels.map(async model => {
          try {
            ...
            console.warn(`[GMDataIngestion] Failed to generate from ${model.modelName}:`, err);
          }
        })
      );
  ...
  // Convert synthetic samples to training data points for GM LLM
  convertToTrainingData(samples: SyntheticDataSample[]): GMMarketDataPoint[] {
