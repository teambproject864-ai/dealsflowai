...
test.describe('Admin Portal - Authentication & Navigation', () => {
  ...
  test('should display all main admin navigation tabs', async ({ authenticatedAdmin }) => {
    ...
    for (const tabName of expectedTabs) {
      await expect(
        authenticatedAdmin.getByRole('button', { name: tabName, exact: true })
      ).toBeVisible();
    }
  });

  test('should switch to GTM Reports tab and display reports', async ({ authenticatedAdmin }) => {
    await authenticatedAdmin.getByRole('button', { name: 'GTM Reports', exact: true }).click({ force: true });
  ...
  test('should switch to Agents tab and show agent management', async ({ authenticatedAdmin }) => {
    await authenticatedAdmin.getByRole('button', { name: 'Agents', exact: true }).click({ force: true });
  ...
  test('should allow admin to onboard a new customer', async ({ authenticatedAdmin }) => {
    ...
    await authenticatedAdmin.getByRole('button', { name: 'Customers', exact: true }).click({ force: true });

    // Click Onboard New Customer button
    await authenticatedAdmin.getByRole('button', { name: 'Onboard New Customer', exact: true }).click({ force: true });
  ...
  test('should allow admin to retrain the AI model', async ({ authenticatedAdmin }) => {
    ...
    await authenticatedAdmin.getByRole('button', { name: 'LLM Manager', exact: true }).click({ force: true });
    ...
    // Click Retrain Model
    await authenticatedAdmin.getByRole('button', { name: 'Retrain Model', exact: true }).click({ force: true });
  });

  test('should switch to Bot Monitor tab and display bot metrics', async ({ authenticatedAdmin }) => {
    // Switch to Bot Monitor tab
    await authenticatedAdmin.getByRole('button', { name: 'Bot Monitor', exact: true }).click({ force: true });
